...
func _physics_process(delta:float) -> void:
	...
	slay_raycast.target_position = velocity * tilemap.rendering_quadrant_size * attack_range

	# Tile centering
	tile_position = ((global_position / (tilemap.rendering_quadrant_size*0.5)) + Vector2(1,1))
	...
	move_and_slide()
	velocity = velocity #used to be velocity.normalized()
...
func jump() -> void:
	jumping = true
	set_collision_mask_value(0, 0) # avoid the Level
	set_collision_mask_value(2, 0) # avoid enemies
	set_collision_layer_value(1, 0) # enemies avoid the player
	jump_timer.start()
	jump_sfx()
...
func _on_JumpTimer_timeout() -> void:
	# Landing
	jumping = false
	set_collision_mask_value(0, 1) # enable level collision
	set_collision_mask_value(2, 4) # enable player-enemy collision
	set_collision_layer_value(1, 2) # enable enemy-player collision
	$Sprite2D.show()
	$FlySprite.hide()
